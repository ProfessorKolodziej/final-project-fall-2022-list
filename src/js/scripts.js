...
function rotateWheel(awardIndex, awardCount) {
    ...
    wheelCanvas.style.transform =
        "rotate(" + (360 - baseAngle * awardIndex + rotateRound + 90) + "deg)";
}
...
function drawWheelCanvas(canvasId, turnWheel, _w = pcWidth, _h = pcWidth) {
    ...
    for (let index = 0; index < turnWheel.reaward.length; index++) {
        ...
        let translateX =
            _w * 0.5 + Math.cos(angle + baseAngle / 2) * turnWheel.textRadius;
        let translateY =
            _h * 0.5 + Math.sin(angle + baseAngle / 2) * turnWheel.textRadius;
